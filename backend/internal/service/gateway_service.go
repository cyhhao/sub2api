...
func (s *GatewayService) applyClaudeCodeOAuthMimicryToBody(
	...
	var systemRewritten bool
	body, systemRewritten = s.rewriteClaudeOAuthMimicSystem(ctx, body, normalizeSystemParam(systemRaw))
	...
	body, _, _ = normalizeClaudeOAuthRequestBody(body, model, normalizeOpts)
...
func (s *GatewayService) rewriteClaudeOAuthMimicSystem(ctx context.Context, body []byte, system any) ([]byte, bool) {
	enabled, prompt, blocks := s.claudeOAuthSystemPromptInjectionSettings(ctx)
	if !enabled {
		return body, false
	}
	return rewriteSystemForNonClaudeCodeWithPromptBlocks(body, system, prompt, blocks), true
}
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if shouldMimicClaudeCode {
		...
		rewrittenBody, systemRewritten := s.rewriteClaudeOAuthMimicSystem(ctx, body, systemRaw)
		...
		normalizedBody, reqModel, toolNameMap = normalizeClaudeOAuthRequestBody(body, reqModel, normalizeOpts)
...
func (s *GatewayService) computeFinalAnthropicBeta(
	...
	if tokenType == "oauth" {
		if mimicClaudeCode {
			...
			// fork 内保持所有 OAuth mimic 模型都使用完整 Claude Code beta set，
			// 避免 haiku 与非 haiku 在下游能力和指纹上分叉。
...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, toolNameMap map[string]string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	processSSEEvent := func(lines []string) ([]string, string, *sseUsagePatch, error) {
		...
		if err := json.Unmarshal([]byte(dataLine), &event); err != nil {
			...
			block += "data: " + replaced + "\n\n"
			return []string{block}, replaced, nil, nil
		}
				...
				if buffered := toolInputBuffers[index]; buffered != "" {
					...
					if synthErr == nil {
						...
						if stopErr == nil {
							...
							return []string{synthBlock, stopBlock}, string(stopBytes), nil, nil
						}
					}
				}
			}
		...
		if err != nil {
			...
			block += "data: " + replaced + "\n\n"
			return []string{block}, replaced, usagePatch, nil
		}
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if shouldMimicClaudeCode {
		...
		var normalizedBody []byte
		normalizedBody, reqModel, _ = normalizeClaudeOAuthRequestBody(body, reqModel, normalizeOpts)
		if err := replaceBody(normalizedBody); err != nil {
			return err
		}
